...
async function publicState(room: Room): Promise<RoomState> {
  ...
  return { code: room.code, phase: room.phase, mapId: room.mapId, budget: room.budget, useStars: room.useStars, defense: room.defense, players, deadline: room.deployDeadline };
}
...
export function attachRooms(io: RoomServer, deps: RoomDeps = { authenticate: userFromSessionCookie, saveMatch, loadPlayer: getPlayer }): void {
  ...
  const syncDeployTimer = (room: Room) => {
    if (room.phase !== 'lobby' || connectedSides(room).length < 2) return void clearDeployTimer(room);
  ...
  const forceStart = async (room: Room) => {
    if (room.phase !== 'lobby' || room.battle) return;
  ...
  io.on('connection', (socket: Sock) => {
    ...
    socket.on('room:create', async (ack) => {
      ...
      const code = newCode();
      room = { code, phase: 'lobby', mapId: map.id, budget: map.budget, useStars: true, defense: null, players: { blue: { uid: user.uid, socketId: socket.id, name: playerName(user), ready: false, army: [], stars: {} } }, battle: null, cleanup: null, deployDeadline: null, deployTimer: null };
      side = 'blue';
      rooms.set(code, room);
    ...
    socket.on('room:join', async (req, ack) => {
      ...
      if (!target) {
        ...
        if (!map) return ack({ ok: false, error: 'Chưa có bản đồ nào trong CMS' });
        leave();
        target = { code, phase: 'lobby', mapId: map.id, budget: map.budget, useStars: true, defense: null, players: { blue: { uid: user.uid, socketId: socket.id, name: playerName(user), ready: false, army: [], stars: {} } }, battle: null, cleanup: null, deployDeadline: null, deployTimer: null };
      ...
      let seat: Side | null = slots.find((s) => target!.players[s]?.uid === user.uid) ?? null;
      if (!seat) seat = slots.find((s) => !target!.players[s]) ?? null;
      if (!seat) return ack({ ok: false, error: target.defense !== null ? 'Phòng đấu thủ thành chỉ có 2 người' : 'Phòng đã đủ 4 người' });
