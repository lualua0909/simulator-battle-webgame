...
export interface RoomsSnapshot {
  ...
  rooms: number;
  lobbies: number;
  battles: Array<{ code: string; blue: string; red: string; mapId: string; siege: boolean; startedAt: number }>;
}
...
function store(): Store {
  if (!g.__battleMetrics) {
    ...
    g.__battleMetrics = {
      ...
      activeUsers: new Map(),
      logins: 0,
      battles: { started: 0, results: { blue: 0, red: 0, draw: 0 }, voided: new Map(), saveFailed: 0 },
      rooms: null,
    };
  }
