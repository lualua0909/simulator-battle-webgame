...
function oldDatabase(): ContentBundle {
  ...
  return {
    ...SEED,
    units: SEED.units.filter((u) => !newUnits.has(u.id)).map((u) => ({ ...u, skillIds: [], hp: u.id === 'wizard' ? 999 : u.hp })),
...
test('lists the default documents, skills and settings a database lacks', () => {
  ...
  assert.deepEqual(missingDefaults(SEED, SEED), { docs: [], unskilled: [], settings: [] });
});

test('merging adds what was picked and keeps admin edits', () => {
  const db = oldDatabase();
  const r = mergeDefaults(db, SEED, { docs: everything(db), skills: true });
  assert.deepEqual(findRefIssues(r.content), []);
  assert.deepEqual(missingDefaults(r.content, SEED), { docs: [], unskilled: [], settings: [] });
...
test('picked documents whose references are gone are skipped, with their dependents', () => {
  ...
  const r = mergeDefaults(noLegends, SEED, { docs: picks, skills: false });
  assert.deepEqual(findRefIssues(r.content), []);
