...
// Downloads a model as TypeScript, glTF/GLB, OBJ, STL, PLY or USDZ. TypeScript exports a
// standalone module with the baked part tree. Mesh exports use flat normals (faceted look).
...
export const EXPORT_FORMATS = [
  { id: 'ts', label: 'TypeScript', hint: 'factory three.js dựng lại model (cây part đã bake)' },
...
/** What to export: any procedural model built on demand. */
export type ExportSource = { build: () => THREE.Object3D; name: string };
...
export async function exportModel(format: ExportFormat, source: ExportSource): Promise<void> {
  ...
  switch (format) {
    case 'ts':
      return download(bakedTypeScript(exportRoot(source, 'float'), source.name), `${name}.ts`, 'text/plain');
    case 'glb': {
